...
<head>
    ...
    <meta property="og:site_name" content="GrowthyTools">
    <meta property="og:locale" content="ar_AR">
    <?php wp_head(); ?>
</head>
<body <?php body_class(); ?>>
<?php wp_body_open(); ?>
<header class="gt-site-header" id="top">
    <div class="gt-container gt-header-inner">
        <div class="gt-logo" aria-label="<?php esc_attr_e('هوية GrowthyTools', 'growthytools'); ?>">
            <?php if (has_custom_logo()) : ?>
                <?php the_custom_logo(); ?>
            <?php else : ?>
                <a href="<?php echo esc_url(home_url('/')); ?>" aria-label="<?php esc_attr_e('العودة إلى الرئيسية', 'growthytools'); ?>">
                    <?php echo esc_html__('GrowthyTools', 'growthytools'); ?>
                </a>
            <?php endif; ?>
        </div>
